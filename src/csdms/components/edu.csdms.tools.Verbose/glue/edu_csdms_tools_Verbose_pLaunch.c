/*
 * File:          edu_csdms_tools_Verbose_pLaunch.c
...
static void (*s_getEPV)
  (struct edu_csdms_tools_Verbose__epv*, struct 
    edu_csdms_tools_Verbose__pre_epv*, struct 
    edu_csdms_tools_Verbose__post_epv*);
...
static struct edu_csdms_tools_Verbose__rmiExternals *s_rmiExternals;
...
initImplementation(void) {
  ...
  if (notInitialized) {
    ...
    {
      sidl_DLL dll = sidl_Loader_findLibrary("edu.csdms.tools.Verbose",
        "python/impl", sidl_Scope_SCLSCOPE,
        sidl_Resolve_SCLRESOLVE, &throwaway_exception);
      if (dll) {
        struct edu_csdms_tools_Verbose__rmiExternals *((*rmi_func)(void));
        notInitialized = 0;
        s_getEPV = (void (*)(struct edu_csdms_tools_Verbose__epv *, struct 
          edu_csdms_tools_Verbose__pre_epv*, struct 
          edu_csdms_tools_Verbose__post_epv*))
          sidl_DLL_lookupSymbol(dll, "edu_csdms_tools_Verbose__impl_set_epv", 
            &throwaway_exception);
        rmi_func = (struct edu_csdms_tools_Verbose__rmiExternals*((*)(void))) 
          sidl_DLL_lookupSymbol(dll, 
            "edu_csdms_tools_Verbose__impl_rmi_externals", 
            &throwaway_exception);
          if (rmi_func) {
...
void
edu_csdms_tools_Verbose__set_epv(struct edu_csdms_tools_Verbose__epv *epv,
    ...
    edu_csdms_tools_Verbose__post_epv *post_epv)
{
  ...
  if (s_getEPV) (*s_getEPV)(epv,pre_epv,post_epv);
    ...
    fputs("\
Babel: Fatal error loading stage 2 Python implementation for edu.csdms.tools.Verbose",
    stderr);
  }
}
#ifdef __cplusplus
...
void edu_csdms_tools_Verbose__call_load(void) { 
...
struct sidl_BaseInterface__object* 
  ...
  return (
    *s_rmiExternals->f_pskel_edu_csdms_tools_Verbose_fconnect_sidl_BaseInterface)
    (url, ar, _ex);
}
